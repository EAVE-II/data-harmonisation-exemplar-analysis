...
m_input <- '/Users/calummacdonald/Usher/NCSI/Elderly/'
...
df_person <- read.csv(paste0(m_input,"data/person.csv"))
...
df_drug_exposure <- read.csv(paste0(m_input,"data/drug_exposure.csv"))
...
df_measurement <- read.csv(paste0(m_input,"data/measurement.csv"))
df_measurement <- df_measurement %>% select(measurement_concept_id,measurement_id,person_id,measurement_date,value_as_number) %>%
                   mutate_at(c('measurement_date'),as.Date)

table(df_measurement$measurement_concept_id)

df_igg <- df_measurement %>% filter(measurement_concept_id == 723474)
df_tcell <- df_measurement %>% filter(measurement_concept_id == 4076842)


df_ana <- df_tcell %>%
          #df_igg %>%
          left_join(df_drug_exposure) %>% left_join(df_person)
...
df_ana_2_pf <- df_ana %>% select(dose,gender_concept_id,
                                 days_since_vaccine,
                                 days_since_vaccineG10,age,
                                 ageG,value_as_number,drug_concept_name) %>%
              mutate(ageG = cut(age,breaks=seq(60,101,7))) %>%
              na.omit %>%
              filter(dose==2 & drug_concept_name=='Pf')
nrow(df_ana_2_pf)
...
fname <- paste0(getwd(),"/results/Elderly/",m_save,".rds")
print (fname)
